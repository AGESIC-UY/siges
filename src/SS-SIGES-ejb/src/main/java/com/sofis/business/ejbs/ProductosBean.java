...
public class ProductosBean {
	...
	private static final Logger LOGGER = Logger.getLogger(ProductosBean.class.getName());
	...
	private Productos guardar(Productos prod, Boolean actualizarPrograma) {
		...
		try {
			...
			entregablesBean.guardarAvanceReal(prod.getProdEntregableFk().getEntPk());
                        ...
                        if(prod.getProdEntregableFk()!=null && prod.getProdEntregableFk().getEntCroFk()!=null){
                            proyectosBean.postProcesarEntregablesRelacionados(prod.getProdEntregableFk().getEntCroFk(), null);
                        }

			Proyectos proy = prod.getProdEntregableFk().getEntCroFk().getProyecto();
			proyectosBean.actualizarIndProyProg(proy.getProyPk(), null, proy.getProyOrgFk().getOrgPk(), actualizarPrograma);
			proyectosBean.actualizarFechaUltimaModificacion(proy.getProyPk());
	...
	public List<Productos> guardarProducto(List<Productos> listProd, Boolean actualizarPrograma) {
		if (CollectionsUtils.isNotEmpty(listProd)) {
			for (Productos prod : listProd) {
				prod = guardarProducto(prod, actualizarPrograma);
			}
		}
		return listProd;
	}
	...
	public Productos guardarProducto(Productos prod, Boolean actualizarPrograma) {
		if (prod != null) {
			...
			return guardar(prod, actualizarPrograma);
		}
		return null;
	}
	...
	private Productos agregarProdMes(Productos prod) {
		if (prod != null && prod.getProdEntregableFk() != null) {
			...
			Date dInicio = ent.getEntInicioDate();
			if (ent.getEntInicioLineaBase() != null && ent.getEntInicioLineaBase() > 0
					&& DatesUtils.esMayor(dInicio, ent.getEntInicioLineaBaseDate())) {
				dInicio = ent.getEntInicioLineaBaseDate();
			}
			calInicio.setTime(dInicio);

			Calendar calFin = GregorianCalendar.getInstance();
			Date dFin = ent.getEntFinDate();
			if (ent.getEntFinLineaBase() != null && ent.getEntFinLineaBase() > 0
					&& DatesUtils.esMayor(ent.getEntFinLineaBaseDate(), dFin)) {
				dFin = ent.getEntFinLineaBaseDate();
			}
			calFin.setTime(dFin);
			...
			if (CollectionsUtils.isNotEmpty(prod.getProdMesList())) {
				...
				for (ProdMes prodMes : prod.getProdMesList()) {
					...
					//Si la fecha del mes no está dentro del entregable se borra el valor real.
					Calendar cal = new GregorianCalendar();
					cal.set(Calendar.YEAR, prodMes.getProdmesAnio());
					cal.set(Calendar.MONTH, prodMes.getProdmesMes() - 1);
					cal.set(Calendar.DAY_OF_MONTH, 1);
					boolean esFechaEnt = DatesUtils.esEntreFechas(cal.getTime(), ent.getEntInicioDate(), ent.getEntFinDate(), "yyyyMM");
					if (!esFechaEnt) {
						prodMes.setProdmesReal(0D);
					}
				}
	...
	public void eliminarProducto(Integer prodPk) {
		if (prodPk != null) {
			...
			try {
				...
				Proyectos proy = prod.getProdEntregableFk().getEntCroFk().getProyecto();
				Integer entPk = prod.getProdEntregableFk().getEntPk();

				dao.delete(prod);

				entregablesBean.guardarAvanceReal(entPk);

				proyectosBean.actualizarIndProyProg(proy.getProyPk(), null, proy.getProyOrgFk().getOrgPk(), true);
				proyectosBean.actualizarFechaUltimaModificacion(proy.getProyPk());
	...
	public void actualizarProdPorEnt(Set<Entregables> colEnt, Boolean actualizarPrograma) {
			...
			for (Entregables ent : colEnt) {
				...
				if (CollectionsUtils.isNotEmpty(prodList)) {
					for (Productos prod : prodList) {
						guardarProducto(prod, actualizarPrograma);
					}
				}
			}
		}
	...
	public Double obtenerTotalAcuPlan(Integer prodPk) {
		ProductosDAO dao = new ProductosDAO(em);
		return dao.obtenerTotalAcuPlan(prodPk);
	}

	public Double obtenerTotalAcuPlan1(Integer prodPk) {
	...
	public String prodMesAcuRealColor(ProdMes prodMes, Integer orgPk, Integer limiteAmarillo, Integer limiteRojo) {
		if (prodMes != null
			...
			} else if ((prodMes.getProdmesAcuPlan() == 0 && prodMes.getProdmesAcuReal() == 0)
					|| (porcAcuReal < 100 && porcAcuReal > (100 - limiteAmarillo))) {
				return ConstantesEstandares.SEMAFORO_VERDE;
		...
		if (ent != null && nvoEnt != null) {
			...
			if (CollectionsUtils.isNotEmpty(prodList)) {
				for (Productos prod : prodList) {
					...
					// No se copia el desglose por mes, en su lugar se crean nuevos sin valores
					agregarProdMes(nvoProd);
	...
	public List<ProdMes> copiarProyProdMes(List<ProdMes> prodMesList, Productos nvoProd, int desfasajeDias) {
		...
		if (CollectionsUtils.isEmpty(prodMesList)) {
			return null;
		}
		...
		for (ProdMes prodMes : prodMesList) {
			...
			Calendar cal = new GregorianCalendar(prodMes.getProdmesAnio(), prodMes.getProdmesMes() + 1, 1);
	...
	public Productos cargarProdMesAuxiliar(Productos prod, Integer anio) {
		if (prod != null) {
			...
			ProdMes ultimo = prod.ultimoProdMes();

			if (ultimo != null && ultimo.getProdmesAnio() < year) {
				year = ultimo.getProdmesAnio();
			}
	...
	public Map<String, Integer> cantPorcProdPorRango(Integer proyPk, Integer limiteAmarilloProd, Integer limiteRojoProd, Integer orgPk) {
		...
		if (listProd != null) {
			for (Productos prod : listProd) {
				ProdMes ultimoProdMes = prod.obtenerProdMes(calNow.get(Calendar.YEAR), calNow.get(Calendar.MONTH));
				if (ultimoProdMes == null) {
					ProdMes ultimoPM = prod.ultimoProdMes();
					if (ultimoPM != null) {
						Calendar calUltimoMes = new GregorianCalendar();
						calUltimoMes.set(ultimoPM.getProdmesAnio(), ultimoPM.getProdmesMes(), 1);
						if (calUltimoMes.before(calNow)) {
							ultimoProdMes = ultimoPM;
						}
					}
				}

				if (ultimoProdMes != null && ultimoProdMes.getProdmesAcuPlan() != null && ultimoProdMes.getProdmesAcuReal() != null) {
					...
					if (porc < 100 && porc >= (100 - limiteAmarilloProd)
						...
						/**
						 * 15-12-2016 FIX BUG 7540 Estaba comparando el
						 * acumulado planificado con el acumulado real del mes
						 * actual y lo tomaba como finalizado. Debe tomarlo como
						 * finalizado si el mes actual es el último mes o en el
						 * último mes el acumulado real es mayor o igual al
						 * acumulado planificado.
						 */
						if (ultimoProdMes.equals(prod.ultimoProdMes())
								|| prod.ultimoProdMes().getProdmesAcuPlan() <= prod.ultimoProdMes().getProdmesAcuReal()) {
	...
	/**
	 * Retorna los productos cuyo mes anterior a hoy el acumulado real 0.
	 *
			...
			if (CollectionsUtils.isNotEmpty(listProd)) {
				...
				for (Productos prod : listProd) {
					...
					if (prodMesCalcular != null
							&& prodMesCalcular.getProdmesAcuPlan() != null && prodMesCalcular.getProdmesAcuPlan() > 0
							&& (prodMesCalcular.getProdmesAcuReal() == null || prodMesCalcular.getProdmesAcuReal() == 0)) {
