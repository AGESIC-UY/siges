...
public class MailBean {
    ...
    public boolean conCorreo(Integer orgPk) {
 ...
 /*try {
    ...
    public void comunicarSolicitudAprobacion(Integer orgPk, Object obj, String... destinatario) throws GeneralException {
        ...
        if (conCorreo(orgPk) && mt != null) {
            ...
            String asunto = MailsTemplateUtils.instanciarConHashMap(mt.getMailTmpAsunto(), valores);
            String mensaje = MailsTemplateUtils.instanciarConHashMap(mt.getMailTmpMensaje(), valores);

            this.enviarMail(asunto, "", destinatario, mensaje, orgPk);
        }
    }

    public void comunicarSolicitudReplanificacion(Integer orgPk, Object obj, String... destinatario) throws GeneralException {
        MailsTemplate mt = mailsTemplateBean.obtenerMailTmpPorCodigo(MailsTemplateBean.MAIL_SOL_REPLANIFICACION, orgPk);
    ...
    private class EnviarMailThread extends Thread {
        ...
        public void run() {
            ...
            try {
                ...
                logger.log(Level.INFO, "## MANDANDO MAIL ---" + "para: " + toAddresses.toString() + "--organizacionPk: " + orgPk + "--motivo:" + mensaje
                );
                MUTEX.acquire();
                Transport.send(msg);
