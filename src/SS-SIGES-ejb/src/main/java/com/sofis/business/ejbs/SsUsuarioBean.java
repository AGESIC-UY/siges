...
public class SsUsuarioBean implements UsuarioLocal {
    ...
    private static final Logger logger = Logger.getLogger(SsUsuarioBean.class.getName());
    ...
    private static final String MUTEX_ID = "LIMPIAR_TOKENS";
    ...
    public void agregarRolUsuario(SsUsuario usuario, SsRol rol, Organismos org) {
        ...
        ofiRoles.setUsuOfiRolesActivo(true);
    ...
    public List<SsUsuario> busquedaUsuFiltro(Organismos org, SsUsuario usu, String mail, String nombre, String apellido, SsRol rol) {
        return busquedaUsuFiltro(org, usu, mail, nombre, apellido, null, rol, null, null);
    }
...
//		MatchCriteriaTO criterio = CriteriaTOUtils.createMatchCriteriaTO(MatchCriteriaTO.types.CONTAINS, "usuPrimerApellido", apellido.toUpperCase().trim());
            ...
            if (rol != null) {
                MatchCriteriaTO criterio = CriteriaTOUtils.createMatchCriteriaTO(
                        MatchCriteriaTO.types.EQUALS, "ssUsuOfiRolesCollection.usuOfiRolesRol.rolId", rol.getRolId());
                criterios.add(criterio);
            }
            ...
            if (elementoOrdenacion != null) {
                orderBy[0] = elementoOrdenacion;
            } else {
                orderBy[0] = "usuCorreoElectronico";
            }

            if (ascendente != null) {
                asc[0] = ascendente.equalsIgnoreCase("Ascendente");
            } else {
                asc[0] = true;
            }
            ...
            if (ev.validate(email)) {
                ...
                if (usu != null) {
                    ...
                    if (orgPk == null) {
                        ...
                        orgPk = buscarOficina(setUsuRoles);

                    } else {

                        if (!mailBean.conCorreo(orgPk)) {
...
//                    be.addError(MensajesNegocio.ERROR_RESET_PASS_USUARIO);
                        ...
                        be.addError("No es posible envíar mail con recuperación de contraseña por favor contactar al administrador de su Organismo");
                        throw be;
                    }
...
//                    be.addError(MensajesNegocio.ERROR_RESET_PASS_USUARIO);
    ...
    @Schedule(hour = HORA_EJECUCION, minute = MINUTOS_EJECUCION, persistent = false, info = "limpiar_tokens")
    ...
    public SsUsuario obtenerSsUsuarioPorLDAPUser(String user) throws GeneralException {
        logger.log(Level.FINEST, "obtenerSsUsuarioPorLDAPUser...");
        if (!StringsUtils.isEmpty(user)) {
            user = user.toLowerCase();
            SsUsuarioDAO dao = new SsUsuarioDAO(em);
            try {
                List<SsUsuario> resultado = dao.findByOneProperty(SsUsuario.class, "usuLdapUser", user);
                return (SsUsuario) DAOUtils.obtenerSingleResult(resultado);
            } catch (DAOGeneralException ex) {
                logger.log(Level.SEVERE, SsUsuarioBean.class.getName(), ex);
                BusinessException be = new BusinessException(ex);
    ...
    private Integer buscarOficina(Set<SsUsuOfiRoles> setUsuRoles) {

        if (setUsuRoles != null && !setUsuRoles.isEmpty()) {

            Iterator<SsUsuOfiRoles> it = setUsuRoles.iterator();

            while (it.hasNext()) {

                Integer ofiId = it.next().getUsuOfiRolesOficina().getOfiId();
                if (mailBean.conCorreo(ofiId)) {
