...
public class TiposMediaBean {
    ...
    public TiposMedia obtenerPorCodigo(String codigo) {
        if (!StringsUtils.isEmpty(codigo)) {
            TiposMediaDAO dao = new TiposMediaDAO(em);
            try {
                List<TiposMedia> result = dao.findByOneProperty(TiposMedia.class, "tipCod", codigo);
                return (TiposMedia) DAOUtils.obtenerSingleResult(result);
            } catch (DAOGeneralException ex) {
                Logger.getLogger(TiposMediaBean.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
        return null;
    }
