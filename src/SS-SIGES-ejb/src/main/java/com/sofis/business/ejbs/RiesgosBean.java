...
public class RiesgosBean {
	...
	public Riesgos guardar(Riesgos riesgo, Integer orgPk) {
		...
		try {
			...
			if (orgPk != null) {
				...
				notificarNuevoRiesgoAlto(riesgo.getRiskProyFk(), orgPk, null);
			}
			
			proyectosBean.actualizarFechaUltimaModificacion(riesgo.getRiskProyFk().getProyPk());
	...
	public void notificarNuevoRiesgoAlto(Proyectos proy, Integer orgPk, Map<String, String> confMap) {
	...
	public void eliminarRiesgo(Integer riskPk, Integer fichaFk, Integer tipoFicha, SsUsuario usuario, Integer orgPk) throws GeneralException {
		...
		try {
			...
			// actualiza la fecha de ultima modificacion de riesgos
			// TODO: mejorar el calculo de indicadores para considerar riesgos eliminados
			riesgo.getRiskProyFk().getProyIndices().setProyindRiesgoUltact(new Date());
	...
	public Riesgos revertirSuperacionRiesgo(Integer riskPk, Integer usuId, Integer orgPk) {
		...
		r.setRiskUsuarioSuperadoFk(null);
		r.setRiskFechaSuperado(null);
	...
	public List<Riesgos> copiarProyRiesgos(List<Riesgos> listRiesgos, Map<Integer, Entregables> entregablesMap, Proyectos proy, int desfasajeDias) {
		if (listRiesgos != null && proy != null) {
			...
			while (iterator.hasNext()) {
				...
				nvoRiesgo.setRiskObservaciones(riesgo.getRiskObservaciones());

				if (riesgo.getRiskEntregable() != null) {
					nvoRiesgo.setRiskEntregable(entregablesMap.get(riesgo.getRiskEntregable().getEntPk()));
				}
