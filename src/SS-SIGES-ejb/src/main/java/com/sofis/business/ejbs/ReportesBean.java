...
//	    MatchCriteriaTO proc = CriteriaTOUtils.createMatchCriteriaTO(MatchCriteriaTO.types.CONTAINS, "proyPreFk.adquisicionSet.adqProcCompra", filtro.getProcedimiento());
	    CriteriaTO proc = DAOUtils.createMatchCriteriaTOString("proyPreFk.adquisicionSet.adqProcCompra", filtro.getProcedimiento());
	    criterios.add(proc);
	}

	//procedimientoGRP;
	if (!StringsUtils.isEmpty(filtro.getProcedimientoGRP())) {
//	    MatchCriteriaTO proc = CriteriaTOUtils.createMatchCriteriaTO(MatchCriteriaTO.types.CONTAINS, "proyPreFk.adquisicionSet.adqProcCompraGrp", filtro.getProcedimientoGRP());
	    CriteriaTO proc = DAOUtils.createMatchCriteriaTOString("proyPreFk.adquisicionSet.adqProcCompraGrp", filtro.getProcedimientoGRP());
	    criterios.add(proc);
	}
...
//	    criterios.add(fuenteA);
	...
	if (filtro.getAnioCronograma() != null) {
	    ...
	    MatchCriteriaTO inicioIniCriteria = CriteriaTOUtils.createMatchCriteriaTO(MatchCriteriaTO.types.GREATEREQUAL, "proyCroFk.entregablesSet.entInicio", calIni.getTimeInMillis());
	    MatchCriteriaTO inicioFinCriteria = CriteriaTOUtils.createMatchCriteriaTO(MatchCriteriaTO.types.LESSEQUAL, "proyCroFk.entregablesSet.entInicio", calFin.getTimeInMillis());
	    MatchCriteriaTO finIniCriteria = CriteriaTOUtils.createMatchCriteriaTO(MatchCriteriaTO.types.GREATEREQUAL, "proyCroFk.entregablesSet.entFin", calIni.getTimeInMillis());
	    MatchCriteriaTO finFinCriteria = CriteriaTOUtils.createMatchCriteriaTO(MatchCriteriaTO.types.LESSEQUAL, "proyCroFk.entregablesSet.entFin", calFin.getTimeInMillis());
	    MatchCriteriaTO inicioAntesCriteria = CriteriaTOUtils.createMatchCriteriaTO(MatchCriteriaTO.types.LESSEQUAL, "proyCroFk.entregablesSet.entInicio", calIni.getTimeInMillis());
	    MatchCriteriaTO finDespuesCriteria = CriteriaTOUtils.createMatchCriteriaTO(MatchCriteriaTO.types.GREATEREQUAL, "proyCroFk.entregablesSet.entFin", calFin.getTimeInMillis());

	    criterios.add(CriteriaTOUtils.createORTO(
		    CriteriaTOUtils.createANDTO(inicioIniCriteria, inicioFinCriteria),
		    CriteriaTOUtils.createANDTO(finIniCriteria, finFinCriteria),
		    // tareas que comienzan antes del año y terminan luego. Por ej: si comienzan en 2015 y terminan en 2017 deben mostrarse en el reporte de 2016
	...
	if (filtro.getAnioBaseCronograma()!= null) {
	    ...
	    MatchCriteriaTO inicioIniCriteria = CriteriaTOUtils.createMatchCriteriaTO(MatchCriteriaTO.types.GREATEREQUAL, "proyCroFk.entregablesSet.entInicioLineaBase", calIni.getTimeInMillis());
	    MatchCriteriaTO inicioFinCriteria = CriteriaTOUtils.createMatchCriteriaTO(MatchCriteriaTO.types.LESSEQUAL, "proyCroFk.entregablesSet.entInicioLineaBase", calFin.getTimeInMillis());
	    MatchCriteriaTO finIniCriteria = CriteriaTOUtils.createMatchCriteriaTO(MatchCriteriaTO.types.GREATEREQUAL, "proyCroFk.entregablesSet.entFinLineaBase", calIni.getTimeInMillis());
	    MatchCriteriaTO finFinCriteria = CriteriaTOUtils.createMatchCriteriaTO(MatchCriteriaTO.types.LESSEQUAL, "proyCroFk.entregablesSet.entFinLineaBase", calFin.getTimeInMillis());
	    MatchCriteriaTO inicioAntesCriteria = CriteriaTOUtils.createMatchCriteriaTO(MatchCriteriaTO.types.LESSEQUAL, "proyCroFk.entregablesSet.entInicioLineaBase", calIni.getTimeInMillis());
	    MatchCriteriaTO finDespuesCriteria = CriteriaTOUtils.createMatchCriteriaTO(MatchCriteriaTO.types.GREATEREQUAL, "proyCroFk.entregablesSet.entFinLineaBase", calFin.getTimeInMillis());

	    criterios.add(CriteriaTOUtils.createORTO(
		    CriteriaTOUtils.createANDTO(inicioIniCriteria, inicioFinCriteria),
		    CriteriaTOUtils.createANDTO(finIniCriteria, finFinCriteria),
		    // L{ineas base que comienzan antes del año y terminan luego. Por ej: si comienzan en 2015 y terminan en 2017 deben mostrarse en el reporte de 2016
		    CriteriaTOUtils.createANDTO(inicioAntesCriteria, finDespuesCriteria)
	    ));
	}
	...
	if (listProy != null) {
		...
		if (proy.getProyPreFk() != null && proy.getProyPreFk().getAdquisicionSet() != null) {
			...
			aplicaFiltro = true;
			if (aplicaFiltro
				&& !StringsUtils.isEmpty(filtro.getProcedimiento())
				&& !StringsUtils.contains(adq.getAdqProcCompra(), filtro.getProcedimiento())) {
			    aplicaFiltro = false;
			}

			if (aplicaFiltro
				&& !StringsUtils.isEmpty(filtro.getProcedimientoGRP())
				&& !StringsUtils.contains(adq.getAdqProcCompraGrp(), filtro.getProcedimientoGRP())) {
			    aplicaFiltro = false;
			}

			if (aplicaFiltro
				&& filtro.getFuenteFinanc() != null && filtro.getFuenteFinanc().getFuePk() != null
				&& adq.getAdqFuente() != null && adq.getAdqFuente().getFuePk() != null
				&& filtro.getFuenteFinanc().getFuePk() != adq.getAdqFuente().getFuePk()) {
			    aplicaFiltro = false;
			}

			if (aplicaFiltro
				&& filtro.getMoneda() != null && filtro.getMoneda().getMonPk() != null
