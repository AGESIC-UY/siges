...
import com.sofis.web.delegates.NoticiaDelegate;
...
public class NoticiaMB implements Serializable {
    ...
    @Inject
    private ConsultaHistoricoDelegate<Noticia> histDelegate;
    @Inject
    private NoticiaDelegate natDelegate;
    ...
    private Noticia notEnEdicion = new Noticia();
    private Boolean renderPopupEdicion = false;
    private List<Noticia> listaHitorial = new ArrayList();
    private Boolean renderPopupHistorial = false;
    private String cantElementosPorPagina = "25";
    private String elementoOrdenacion = "notCodigo";
    ...
    public String buscar() {
        ...
        if (!StringsUtils.isEmpty(bCodigo)) {
            MatchCriteriaTO criterio = CriteriaTOUtils.createMatchCriteriaTO(
                    MatchCriteriaTO.types.CONTAINS, "notCodigo",
        ...
        if (!StringsUtils.isEmpty(bDescripcion)) {
            MatchCriteriaTO criterio = CriteriaTOUtils.createMatchCriteriaTO(
                    MatchCriteriaTO.types.CONTAINS, "notContenido",
        ...
        if (!criterios.isEmpty()) {
            ...
            condicion = CriteriaTOUtils.createMatchCriteriaTO(
                    MatchCriteriaTO.types.NOT_NULL, "notId", 1);
        }
        String[] propiedades = {"notId", "notCodigo", "notContenido"};
        String className = Noticia.class.getName();
    ...
    public String consultarHistorial(Integer id) {
        try {
            listaHitorial = histDelegate.obtenerHistorialPorId(Noticia.class, id, "notVersion");
    ...
    public String guardar() {
        try {
            natDelegate.guardar(notEnEdicion);
...
//        reset();
    ...
    public Noticia getNotEnEdicion() {
        return notEnEdicion;
    }

    public void setNotEnEdicion(Noticia notEnEdicion) {
        this.notEnEdicion = notEnEdicion;
    }
    ...
    public List<Noticia> getListaHitorial() {
        return listaHitorial;
    }

    public void setListaHitorial(List<Noticia> listaHitorial) {
