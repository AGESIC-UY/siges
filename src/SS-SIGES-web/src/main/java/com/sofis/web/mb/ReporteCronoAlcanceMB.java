...
public class ReporteCronoAlcanceMB implements Serializable {
	...
	private void cargarCombosFiltro() {
		...
		Date primeraDate = entregablesDelegate.obtenerPrimeraFecha(inicioMB.getOrganismo().getOrgPk());
		if (primeraDate != null) {
		...
		Date ultimaDate = entregablesDelegate.obtenerUltimaFecha(inicioMB.getOrganismo().getOrgPk());
		if (ultimaDate != null) {
	...
	public String areaTematicaPopup(Boolean renderPopup) {
		...
		try {
			...
			if (listaAreasTags != null && !listaAreasTags.isEmpty() && areasTematicasStateMap == null) {
				...
				Map<String, Object> mapAreaTag = AreaTematicaUtils.setNodosForAreaTematica(listaAreasTags, listaAreasTagsTreeNode, areasTematicas, false);
	...
	public String exportarAlcanceAction() {
		...
		Matomo.callTrackDownload(ConstantesPresentacion.REPORTE_ALCANCE);
	...
	private void cargarCombosSeleccionados() {
		...
		filtro.setAnioCronograma((Integer) anioCombo.getSelectedT().getValue());
		filtro.setAnioBaseCronograma((Integer) anioCombo.getSelectedT().getValue());
	}
	...
	public String exportarCronogramaAction() {
		...
		Matomo.callTrackDownload(ConstantesPresentacion.REPORTE_CRONOGRAMA);
