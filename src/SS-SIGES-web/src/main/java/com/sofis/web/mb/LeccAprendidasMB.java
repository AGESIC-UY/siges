...
public class LeccAprendidasMB implements Serializable {
    ...
    private List<MutableTreeNode> listaAreasConTreeNodeFiltro = new ArrayList<>();
	...
	try {
		...
		Map<String, Object> mapAreasTag = WebUtils.setNodosForAreaTematica(listaAreasTags, listaAreasTagsTreeNode, areasTematicas, areasTematicasStateMap);
...
//                rendered = false;
	...
	if (filtro != null) {
		...
		MatchCriteriaTO orga = CriteriaTOUtils.createMatchCriteriaTO(MatchCriteriaTO.types.EQUALS, "lecaprOrgFk.orgPk", filtro.getOrgPk());
		criterias.add(orga);
	    }
		...
		CriteriaTO criteriaActivoFalse = CriteriaTOUtils.createMatchCriteriaTO(MatchCriteriaTO.types.EQUALS, "activo", Boolean.FALSE);
