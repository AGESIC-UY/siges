...
function openBlackPopup(url,width,height,onCloseCallBack,iframeId){
  ...
  if(window.name!=iframeId)
    bg.css({backgroundImage:"url('"+contextPath+"../robicch-jQueryGantt/fondoPopup.png')"});
  ...
  bg.find("iframe:first").attr("src",url).css({width:width, height:height,top:100});

  var bdiv= $("<div>").css({width:width,position:"relative",height:"5px", textAlign:"right", margin:"auto" });
  bdiv.append("<img src=\"../img/eliminar.png\" style='cursor:pointer;position:absolute;right:30px;top:10px;height:35px;'>");
...
function createBlackPage(width,height,onCloseCallBack){
  ...
  var bg=$("<div>").attr("id","__blackpopup__");
  bg.css({position:'fixed',top:"0px",paddingTop:"50px", left:0,width:'100%',height:'100%',  backgroundImage:"url('../robicch-jQueryGantt/fondoPopup.png')"});
  bg.append("<div id='bwinPopupd' name='bwinPopupd'></div>");
  
  bg.bringToFront();

  var ret=bg.find("#bwinPopupd");
  ret.css({width:width,top:10,overflow:'auto', margin:"auto" });

  var bdiv= $("<div>").css({width:width,position:"relative",height:"0px", textAlign:"right", margin:"auto" });
//  var img=$("<img src='../img/eliminar.png' style='cursor:pointer;position:absolute;right:30px;top:10px;height:35px;' title='cerrar'>");
  var img=$("<img src='../img/eliminar.png' style='cursor:pointer;position:absolute;right:-20px;top:12px;' title='cerrar'>");
  bdiv.append(img);
  img.click( function(){
    bg.trigger("close");
  });
