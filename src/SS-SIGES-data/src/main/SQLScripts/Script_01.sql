...
ALTER TABLE `proyectos` ADD COLUMN `proy_factor_impacto` TEXT NULL DEFAULT NULL AFTER `proy_situacion_actual`;
ALTER TABLE `programas` ADD COLUMN `prog_factor_impacto` TEXT NULL DEFAULT NULL AFTER `prog_obj_publico`;
...
DELETE FROM aud_doc_file;
ALTER TABLE doc_file DROP COLUMN docfile_file;


-- Desde acá en adelante es v4.4-12

BEGIN;
DELETE FROM media_proyectos WHERE media_tipo_fk IS NULL AND media_pk > 0;
ALTER TABLE media_proyectos DROP FOREIGN KEY media_tipo_fk;
ALTER TABLE media_proyectos MODIFY media_tipo_fk INT(11) NOT NULL default 1;
ALTER TABLE media_proyectos ADD CONSTRAINT media_tipo_fk FOREIGN KEY (media_tipo_fk) REFERENCES tipos_media (tip_pk) ON DELETE NO ACTION ON UPDATE NO ACTION;
COMMIT;
