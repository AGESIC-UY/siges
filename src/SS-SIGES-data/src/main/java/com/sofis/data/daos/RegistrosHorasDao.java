...
public class RegistrosHorasDao extends HibernateJpaDAOImp<RegistrosHoras, Integer> implements Serializable {
    ...
    public List<Moneda> obtenerMonedasPorProy(Integer proyPk, String db) {
        if (proyPk != null) {
            String queryStr = "SELECT v.valHorMonedaFk"
            ...
            if (db != null && db.trim().equalsIgnoreCase(ConstanteApp.DB_POSTGRESQL)) {
                queryStr = "SELECT m"
                        + " FROM RegistrosHoras h, ValorHora v, Moneda m"
                        + " WHERE h.rhUsuarioFk.usuId = v.valHorUsuarioFk.usuId"
                        + " AND m.monPk = v.valHorMonedaFk.monPk"
                        + " AND h.rhProyectoFk.proyPk = :proyPk"
                        + " GROUP BY m.monPk";
            }
    ...
    public boolean tieneDependenciasEnt(Integer entPk) {
        if (entPk != null) {
            String queryStr = "SELECT COUNT(h.rhPk) AS cant"
                    + " FROM RegistrosHoras h"
