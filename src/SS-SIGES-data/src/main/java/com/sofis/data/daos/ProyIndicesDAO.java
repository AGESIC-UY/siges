...
public class ProyIndicesDAO extends HibernateJpaDAOImp<ProyIndices, Integer> implements Serializable {
    ...
    private static final Logger logger = Logger.getLogger(ProyIndicesDAO.class.getName());       
    ...
    public Date ultimaActualizacion(Integer proyPk) {
        if (proyPk != null) {
            String queryStr = "SELECT p.proyIndices.proyindFechaAct FROM Proyectos p WHERE p.proyPk = :proyPk";
            Query query = getEm().createQuery(queryStr);
            query.setParameter("proyPk", proyPk);
            try {
