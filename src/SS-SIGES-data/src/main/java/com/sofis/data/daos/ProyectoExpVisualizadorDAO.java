...
//	    MatchCriteriaTO criterioNombre = CriteriaTOUtils.createMatchCriteriaTO(MatchCriteriaTO.types.CONTAINS, "proyNombre", filtro.getNombre());
	...
	if (filtro.getFechaDesde() != null) {
	    MatchCriteriaTO criterioDesde = CriteriaTOUtils.createMatchCriteriaTO(
		    MatchCriteriaTO.types.GREATEREQUAL, "proyFechaAct", filtro.getFechaDesde());
	    citeriaList.add(criterioDesde);
	}

	if (filtro.getFechaHasta() != null) {
	    MatchCriteriaTO criterioHasta = CriteriaTOUtils.createMatchCriteriaTO(
		    MatchCriteriaTO.types.LESSEQUAL, "proyFechaAct", filtro.getFechaHasta());
	...
	if (filtro.getEstadoPublicacion() != null) {
		...
		MatchCriteriaTO criterioFechaPub = CriteriaTOUtils.createMatchCriteriaTO(
			MatchCriteriaTO.types.SUBQUERY, "proyFechaAct", " >= (select CASE WHEN (max(h.proyPublicaFecha) is null) THEN '1971-01-01' ELSE max(h.proyPublicaFecha) end from ProyPublicaHist h where h.proyectoFk = a.proyPk)");
		CriteriaTO pubFecha = CriteriaTOUtils.createANDTO(criterioPublicados, criterioFechaPub);

		citeriaList.add(CriteriaTOUtils.createORTO(criterioEP, pubFecha));
