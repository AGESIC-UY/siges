...
public class ReporteProyectoMB implements Serializable {
    ...
    private static final int MARGEN_INICIO = 318;
    ...
    public void init() {
        ...
        Integer reporteProyPk = (Integer) WebUtils.getFlashContext(REPORTE_PROY_PK);
        ...
        if (obj != null) {
            ...
            if (inicio != null && fin != null) {
                ...
                while (calAuxIni.get(Calendar.YEAR) <= calFin.get(Calendar.YEAR)) {
                    ...
                    if (calAuxIni.get(Calendar.YEAR) == calIni.get(Calendar.YEAR)
                            && calAuxIni.get(Calendar.MONTH) < calIni.get(Calendar.MONTH)) {
                        ReporteAcumuladoMesTO acuMes = new ReporteAcumuladoMesTO(anioP, mesP, 0D, null);
                        ultimoMes = acuMes;
                        acumulado.getMeses().add(acuMes);

                    } else if ((calAuxIni.get(Calendar.YEAR) > calIni.get(Calendar.YEAR)
                            || (calAuxIni.get(Calendar.YEAR) == calIni.get(Calendar.YEAR)
                            && calAuxIni.get(Calendar.MONTH) >= calIni.get(Calendar.MONTH)))
                            && (calAuxIni.get(Calendar.YEAR) <= calFin.get(Calendar.YEAR))) {
                        ...
                        pvMes += presupuestoDelegate.obtenerPVPorMoneda(presupuesto.getPrePk(), adqPk, mon, anioP, mesP);
                        if (acCal.get(Calendar.YEAR) < cal.get(Calendar.YEAR)
                                || (acCal.get(Calendar.YEAR) == cal.get(Calendar.YEAR) && acCal.get(Calendar.MONTH) <= cal.get(Calendar.MONTH))) {
                            acMes += presupuestoDelegate.obtenerACPorMoneda(presupuesto.getPrePk(), adqPk, mon, anioP, mesP);
                            prNegativo = presupuestoDelegate.obtenerPRAtrasado(presupuesto.getPrePk(), adqPk, mon, anioP, mesP);
                        } else {
                            acMes = 0D;
                            prNegativo = false;
                        }
                        prMes += presupuestoDelegate.obtenerPRPorMoneda(presupuesto.getPrePk(), adqPk, mon, anioP, mesP);
                        ...
                        String colorR = presupuestoDelegate.obtenerColorACMensual(pvMes, acMes, prMes, orgPk, limiteAmarilloPre, limiteRojoPre, acCal);

                        ReporteAcumuladoMesTO acuMes = new ReporteAcumuladoMesTO(anioP, mesP, null, pvMes, acMes, prMes, prNegativo, colorR);
                        ultimoMes = acuMes;
                        acumulado.getMeses().add(acuMes);

                    } else {

                        ReporteAcumuladoMesTO acuMes = new ReporteAcumuladoMesTO(anioP, mesP, null, ultimoMes.getValorPlan(), null, ultimoMes.getValorProyectado(), false, ConstantesEstandares.COLOR_TRANSPARENT);
                        ultimoMes = acuMes;
                        acumulado.getMeses().add(acuMes);
                    }
                    ...
                    for (Moneda moneda : listMonedas) {
                        ...
                        while (calAux.get(Calendar.YEAR) <= calFin.get(Calendar.YEAR)) {
                            ...
                            if (calAux.get(Calendar.YEAR) == calIni.get(Calendar.YEAR)
                                ...
                                ReporteAcumuladoMesTO acuMes = new ReporteAcumuladoMesTO(anioH, mesH, null, ultimoMes.getValorPlan(), ultimoMes.getValorReal(), ConstantesEstandares.COLOR_TRANSPARENT);
                                ultimoMes = acuMes;
                                acumulado.getMeses().add(acuMes);
    ...
    private void cargarTablaAlcance(Integer proyPk) {
        if (proyPk != null) {
            ...
            List<ReporteAcumuladoMesTO> croAlcanceAcu = entregablesDelegate.obtenerAcumuladoCroPorMes(entregables);
    ...
    private void cargarTablaCostos(Integer proyPk) {
        ...
        listaCostosMonedas = presupuestoDelegate.obtenerMonedasPresupuesto(pre.getPrePk());
