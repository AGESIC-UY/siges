    ...
    public Configuracion guardar(Configuracion cnf) throws GeneralException {
        ...
        try {
            ...
            if (ConfiguracionValidacion.validar(cnf)) {
                ...
                if (cnf.getId() == null) {
                    Configuracion cnfPersit = obtenerCnfPorCodigoYOrg(cnf.getCnfCodigo(), cnf.getCnfOrgFk().getOrgPk());
                    if (cnfPersit != null) {
...
//           logger.log(Level.SEVERE, ex.getMessage() , ex);
    ...
    public Configuracion obtenerCnfPorCodigoYOrg(String codigo, Integer orgPk) throws GeneralException {
        if (!StringsUtils.isEmpty(codigo) && orgPk != null) {
            ConfiguracionDAO cnfDao = new ConfiguracionDAO(em);
            try {
        ...
        if (organismos != null) {
            for (Organismos org : organismos) {
                ...
                Configuracion[] confArr = new Configuracion[]{
                    ...
                    new Configuracion(ConfiguracionCodigos.TAMANIO_MAX_ARCHIVO_MULTIMEDIA, "", "10485760", org, new Date(), 1),
                    new Configuracion(ConfiguracionCodigos.COSTO_ACTUAL_LIMITE_AMARILLO, "", "10", org, new Date(), 1),
                    new Configuracion(ConfiguracionCodigos.COSTO_ACTUAL_LIMITE_ROJO, "", "10", org, new Date(), 1),
                    ...
                    new Configuracion(ConfiguracionCodigos.CON_CORREO, "Si se envía correo o no", "false", org, new Date(), 1),
                    ...
                    new Configuracion(ConfiguracionCodigos.FOLDER_MEDIA, "Carpeta donde se almacenan los archivos Multimedia", "/siges/media_files/", org, new Date(), 1)
                };
