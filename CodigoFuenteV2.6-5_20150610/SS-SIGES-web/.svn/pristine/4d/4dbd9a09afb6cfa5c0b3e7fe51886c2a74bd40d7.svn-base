...
//        avanceDisabled = false;
        ...
        listaProyectos = participantesDelegate.obtenerProyectosPorUsuarioParticipanteActivo(usuario.getUsuId());
    ...
    public void listarHistorico() {
        if (verHistorico == null || !verHistorico) {
            verHistorico = true;
        } else {
            verHistorico = false;
        }
        ...
        try {
            if (tipoRegistro == 1) {
                ...
                if (CollectionsUtils.isNotEmpty(productosList)) {
                    try {
                        productosList = productosDelegate.guardarProducto(productosList);
    ...
    public void entFormComboChangeListener(ValueChangeEvent event) {
        ...
        if (phaseId.equals(PhaseId.ANY_PHASE)) {
            ...
            if (entSelected != null && entSelected.getEntPk() != null) {
                ...
                avanceDisabled = !(isGerente || isCoordinador) || entSelected.getEntProgreso() > 100
    ...
    public String guardarCalidadAction() {
        if (CollectionsUtils.isNotEmpty(listCalidad)) {
            try {
                ...
                listCalidad = calidadDelegate.guardar(listCalidad, inicioMB.getOrganismo().getOrgPk());
...
//                inicioMB.setRenderPopupMensajes(true);
    ...
    public String retrocederAnioProdMes() {
        if (contieneProdMesAnioMenor()) {
            anioProd = anioProd - 1;
//            productosList = productosDelegate.obtenerProdPorProyPk(registroHoras.getRhProyectoFk().getProyPk());
            productosList = productosDelegate.obtenerProdPorEnt(registroHoras.getRhProyectoFk().getProyPk());
            for (Productos prod : productosList) {
    ...
    public String avanzarAnioProdMes() {
